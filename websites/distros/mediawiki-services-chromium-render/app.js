...
function initApp(options) {
    ...
    if (!app.conf.log_header_whitelist) {
        app.conf.log_header_whitelist = [
            ...
            'user-agent', 'x-request-id', 'cookie'
        ];
    }
    ...
    // res.header('Cookie', 'my_wiki_wiki_UserName=Arsenii; VEE=visualeditor; mw_installer_session=b1oc08tasm2eahrpc4a4q0p4j6; my_wiki_wiki__session=lv07g08b73bj272q21im14qn6usrvjbr; my_wiki_wiki_UserID=1');

    // // Если куки существуют, передаем их как строку
    // let cookies = req.headers.cookie || '';

    // // Конфигурация запроса к MediaWiki с куками
    // const mediaWikiUrl = `http://68.183.34.95/index.php/${req.params.title}`;
    // const options = {
    //     method: 'GET',
    //     url: mediaWikiUrl,
    //     headers: {
    //         'Cookie': cookies,  // Передаем куки как строку
    ...
    // // Делаем запрос к MediaWiki с куками
    // axios(options)
    //     .then(response => {
    //         // Обрабатываем ответ от MediaWiki и передаем его в Proton для генерации PDF
    //         req.mediaWikiHtml = response.data;
    //         next();
    //     })
    //     .catch(error => {
