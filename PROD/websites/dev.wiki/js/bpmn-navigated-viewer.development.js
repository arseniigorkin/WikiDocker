/*!
 * bpmn-js - bpmn-navigated-viewer v12.1.1
 *
...
(function (global, factory) {
  ...
  function SelectionVisuals(canvas, eventBus, selection) {
    ...
    eventBus.on('element.hover', function(event) {
      ...
      // // Добавить документацию для тултипа в объект
      // if (event.element.businessObject !== undefined){
      //   // console.log(event.element.businessObject.documentation);
      //   if (event.element.businessObject.documentation !== undefined) {
      //     // attr$1(gfx, 'data-documentation', event.element.businessObject.documentation[0].text);
      //     attr$1(event.gfx, 'title', event.element.businessObject.documentation[0].text);
      //     console.log(event.element.businessObject.documentation[0].text);
           

      //     // $("[data-element-id="+ event.element.id +"]").tooltip({
      //         // content: "Awesome title!"//(function(){
      //         // console.log("77777");
      //     // });
    ...
    eventBus.on('element.out', function(event) {
      ...
      // // Убираем документацию для тултипа
      // if (event.element.businessObject !== undefined){
      //   // console.log(event.element.businessObject.documentation);
      //   if (event.element.businessObject.documentation !== undefined) {
      //       // attr$1(gfx, 'data-documentation', event.element.businessObject.documentation[0].text);
  ...
  function getPlaneIdFromShape(element) {
    ...
    if (is$1(element, 'bpmn:CallActivity')) {
      return addPlaneSuffix(element.businessObject.extensionElements.values[0].processId);
    }
...
///////////////////////////////////////////////
///////////////////////////////////////////////
// Здесь начинаем корректировку под CallActivity.
// Участвуют: canDrillDown, addOverlay и getPlaneIdFromShape
// и закомментирован classes(overlay.html).toggle(EMPTY_MARKER, !hasContent); для того чтобы 
// выводились ссылки всегда, независимо от наличия контента внутри
///////////////////////////////////////////////
///////////////////////////////////////////////
///////////////////////////////////////////////
  ...
  DrilldownOverlayBehavior.prototype.canDrillDown = function(element) {
    ...
    // return is$1(element, 'bpmn:SubProcess') && canvas.findRoot(getPlaneIdFromShape(element));
        if (is$1(element, 'bpmn:SubProcess') && canvas.findRoot(getPlaneIdFromShape(element))){
          return true;
        }
        else if(is$1(element, 'bpmn:CallActivity')){
          return true;
        }
  };
...
// console.log(this);
    ...
    // Для CallActivity выводим спец. ссылку
    var button;
    if(is$1(element, 'bpmn:CallActivity')){
      button = domify$1('<button class="bjs-drilldown">' + ARROW_DOWN_SVG + '</button>');
      button.addEventListener('click', function() {
        var origUrl = window.location.href;
        var currentHref = origUrl.replace(/((?:(?:pid=[\w\-\.]+\&?)(?<!\?))|(?:(?:\&?pid=[\w\-\.]+)))/, "");
        // console.log(currentHref);
        window.location.href = currentHref + "&pid=" + element.businessObject.extensionElements.values[0].processId;
      });
    }
    else {
      button = domify$1('<button class="bjs-drilldown">' + ARROW_DOWN_SVG + '</button>');
  ...
  Canvas.prototype.setRootElement = function(rootElement, override) {
    ...
    if (!rootElement) {
      // alert("\u{1F517} (Под)Процесс не существует или указана ссылка на развёрнутый подпроцесс.\nДопускаются ссылки только на свёрнутые подпроцессы.");
  ...
  ElementRegistry.prototype.add = function(element, gfx, secondaryGfx) {
    ...
    if (element.businessObject !== undefined){
      // console.log(event.element.businessObject.documentation);
      if (element.businessObject.documentation !== undefined && element.di.bounds !== undefined) {
        // console.log(element);
        var docText = element.businessObject.documentation[0].text.replaceAll("\n", '<br />');
        docText = docText.replaceAll('"', '&quot;');
        docText = docText.replaceAll('“', '&ldquo;');
        docText = docText.replaceAll('”', '&rdquo;');
        docText = docText.replaceAll('′', '&prime;');
        docText = docText.replaceAll('‘', '&lsquo;');
        docText = docText.replaceAll('’', '&rsquo;');
        docText = docText.replaceAll('«', '&laquo;');
        docText = docText.replaceAll('»', '&raquo;');
        docText = docText.replaceAll('″', '&Prime;');
        docText = docText.replaceAll('/\'/', '&lsquo;');
        docText = docText.replaceAll('`', '&#96;');
        // console.log(docText);
        attr$1(gfx, 'data-documentation', docText);

        // Вычисляем цетр тултипа
        // console.log(element.di.bounds);
        var x_pos = element.di.bounds.x + element.di.bounds.width / 2;
        var y_pos = element.di.bounds.y + element.di.bounds.height / 2;
        attr$1(gfx, 'data-tt-x_pos', x_pos); // горизонтальный центр
        attr$1(gfx, 'data-tt-y_pos', y_pos); // вертикальный центр
        // console.log(element.businessObject.documentation[0].text);
      }
    }      
  ...
  /**
   * Create an event recognized by the event bus.
   *
