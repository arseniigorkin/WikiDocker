 ...
 * @type {Object}
 */
const MOBILE_DEVICE_VIEWPORT = puppeteer.devices['Galaxy S III'].viewport;
/**
...
class Renderer {
     ...
     * @return {<BBPromise<Object>>} Promise which resolves with PDF buffer
     */
    articleToPdf(url, format, headers) {
        ...
        url = url.replace(/\?title=$/, '?useskin=vector');
        ...
        return new BBPromise((resolve, reject) => {
            puppeteer.launch(this._puppeteerOptions).then((browser) => {
                ...
                .then((response) => {
                    ...
                    const pdfOptions = Object.assign({}, this._pdfOptions);
                    ...
                    return page.evaluate(() => {
                        ...
                        // Возвращаем содержимое src или целиком элемент iframe, если он найден
                        return iframe ? iframe.src : null;
                    })
                        .then((src) => {
                            if (!src) {
                                console.log("Iframe not found. Proceeding without screenshot.");
                                return null;
                            }
                            ...
                            return page.setViewport({
                                width: 670,
                                height: 400
                            }).then(() => page.goto(iframeSrc, { waitUntil: 'networkidle0' }));
                        })
                        .then((result) => {
                            if (result === null) {
                        ...
                        .then((base64Screenshot) => {
                            ...
                            // Если скриншота нет, возвращаем пустой Promise, чтобы не делать лишний переход
                            return Promise.resolve();
                        })
                        .then(() => {
                            if (screenshotBase64) {
                                return page.evaluate((screenshotBase64, iframeSrc) => {
                                    const diagramDiv = document.getElementById('bpmn_diagram');
                                    if (diagramDiv) {
                                        diagramDiv.innerHTML = `<a href="${iframeSrc}" target="_blank"><img src="data:image/png;base64,${screenshotBase64}" /></a>`;
                                    }
                                }, screenshotBase64, iframeSrc);
                            }
                            return Promise.resolve(); // Если нет скриншота, ничего не делаем
                        })
